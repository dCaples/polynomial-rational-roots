...
def synthetic_division(divide_root, coefficients):
    ...
    while k <= last_element_index:
        if k == 0:
            out_array.append(Fraction(Number(coefficients[k]), Number(1)))
            k+=1
            ...
            out_array.append(add(multiply(out_array[-1], divide_root) , Number(coefficients[k])))
            k+=1
    ...
    # else:
        ...
        # out_array.pop()
        # return out_array
    # print numerator and denominator of each element in out_array
    if out_array[-1].numerator.value != 0:
        return False
    else:
        out_array.pop()
...
out_array = synthetic_division(Number(1), [1, 2, 1])
